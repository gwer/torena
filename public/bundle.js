...
const torena = __webpack_require__(28);

const Train = __webpack_require__(31);
const Datepick = __webpack_require__(33);
...
class Main extends React.Component {
    ...
    render() {
        return React.createElement(
            'div',
            { className: 'main' },
            React.createElement(Datepick, { date: this.state.date, onChange: this.onDateChange.bind(this) }),
            React.createElement(Train, { date: this.state.date, train: this.state.train })
        );
    }
...
module.exports = {
    sections: [{
        ...
        exercises: [{
            ...
            name: 'Присед в уступающем режиме',
            reps: {
                min: 4,
                max: 8
            },
            ...
            name: 'Становая тяга в уступающем режиме',
            reps: {
                min: 4,
                max: 8
            },
        ...
        exercises: [{
            ...
            name: 'Подъёмы гантелей через стороны',
            reps: {
                min: 6,
                max: 12
            },
            ...
            name: 'Подъёмы гантелей через стороны в наклоне',
            reps: {
                min: 6,
                max: 12
            },
...
const deepClone = __webpack_require__(30).deepClone;
...
class Torena {
    ...
    constructor(config, date) {
        ...
        return {
            sections: config.sections.map(s => this.chooseExercises(deepClone(s)))
        };
    }
    ...
    chooseExercises(section) {
        return {
            name: section.name,
            exercises: Array(section.count).fill().map(() => this.pullRandomExercise(section))
        };
    }
    ...
    pullRandomExercise({ exercises }) {
        ...
        const prioritySum = exercises.reduce((acc, ex) => acc + ex.priority, 0);
...
class Train extends React.Component {
    render() {
        const flatTrain = this.props.train.sections.reduce((acc, section) => acc.concat(section.exercises), []);
        ...
        return React.createElement(
            ...
            React.createElement(
                "div",
                { className: "train__date" },
                this.props.date.toLocaleDateString()
            ),
            flatTrain.map((ex, inx) => React.createElement(
                "div",
                { className: "train__exercise", key: inx },
                React.createElement(
                    "div",
                    { className: "train__exercise-name" },
                    ex.name
                ),
                React.createElement(
                    "div",
                    { className: "train__exercise-reps" },
                    ex.reps.min,
                    " \u2014 ",
                    ex.reps.max,
                    " \u043F\u043E\u0432\u0442\u043E\u0440\u0435\u043D\u0438\u0439"
